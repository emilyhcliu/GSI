...
!$$$
   ...
   use module_fv3gfs_ncio, only: Dataset, Variable, Dimension, open_dataset,&
                           close_dataset, get_dim, read_vardata,get_idate_from_time_units
   use gfsreadmod, only: general_reload2  !emily
   use ncepnems_io, only: imp_physics     !emily
   ...
   real(r_kind),pointer,dimension(:,:,:)     :: g_vor,g_div,&
                                                g_q,g_oz,g_tv                 !emily
                                             !  g_cwmr,g_q,g_oz,g_tv          !orig
   real(r_kind),pointer,dimension(:,:,:)     :: g_ql,g_qi,g_qr,g_qs,g_qg,g_cf !emily     
   ...
   integer(i_kind):: iret,nlatm2,nlevs,icm,nord_int
   integer(i_kind):: i,j,k,icount,kk,kr
   integer(i_kind) :: ier,istatus,istatus1,iredundant  !emily
   integer(i_kind) :: latb, lonb, levs
   integer(i_kind),dimension(npe)::ilev,iflag,mype_use
   integer(i_kind),dimension(6):: idate
   integer(i_kind),dimension(4):: odate
...
666 format(a40,2x,2(ES25.18))  !emily
   if ( icount == icm ) then
!>>emily
!     call general_reload(grd,g_z,g_ps,g_tv,g_vor,g_div,g_u,g_v,g_q,g_oz,g_cwmr, &
!          icount,iflag,ilev,work,uvflag,vordivflag)
...
!<<emily
   ...
   do k=1,nlevs
      ...
      if (mype==mype_use(icount)) then
         call read_vardata(atmges, 'clwmr', rwork3d0, nslice=kr, slicedim=3)
!        call read_vardata(atmges, 'icmr', rwork3d1, nslice=kr, slicedim=3) !orig
!        ! Cloud condensate mixing ratio.           !orig
!        rwork2d = rwork3d0(:,:,1)+rwork3d1(:,:,1)  !orig
         ! Cloud liquid water mixing ratio.         !emily
         if ( diff_res ) then
!           grid_b=rwork2d          !orig
            grid_b=rwork3d0(:,:,1)  !emily
            vector(1)=.false.
            call fill2_ns(grid_b,grid_c(:,:,1),latb+2,lonb)
            call g_egrid2agrid(p_high,grid_c,grid2,1,1,vector)
            do kk=1,grd%itotsub
...
667 format(a40,2x,i6,2x,2(ES25.18,2x)) !emily
