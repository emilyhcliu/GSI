...
! Declare local parameters
  ...
  !sieron change from 13 to 14 for sacv
...
!    Reopen unit to satellite bufr file
     ...
     ! sieron 02/21 support multiple spc coefficient files for any given sensor
     ! sieron redo indentation
     if(amsua .or. amsub .or. mhs)then
        ...
        do while (spc_coeff_found)
           ...
           !write(*,*) trim(crtm_coeffs_path)
           !write(*,*) trim(sensorlist(1))
           spc_filename = trim(crtm_coeffs_path) // trim(sensorlist(1)) // '.SpcCoeff.bin'
           !write(*,*) 'spc_filename: ', trim(spc_filename)
           INQUIRE(FILE=trim(spc_filename), EXIST=spc_coeff_found)
           !write(*,*) 'spc_coeff_found: ', spc_coeff_found           
...
!          sounders.
!          sieron: changed from accepting TMBR and converting TMBRST to
...
!
           if (llll == 1) then
              ...
              if ( amsua .or. amsub .or. mhs .AND. spc_coeff_versions .ne. 1 .AND. sacv .ne. 1)then
...
!       sieron no need to deallocate crtm info
        ! error_status = crtm_spccoeff_destroy()
        ! if (error_status /= success) &
        !    write(6,*)'OBSERVER:  ***ERROR*** crtm_spccoeff_destroy error_status=',error_status
     end if
