...
!-------------------------------------------------------------------------
    ...
    real(r_kind),pointer,dimension(:,:,:) :: sub_qr, sub_qs, sub_qg !emily
    ...
    real(r_kind),dimension(grd%lat1,grd%lon1,grd%nsig):: qism, qlsm, qrsm, qssm, qgsm !emily
    ...
    integer(i_kind) :: lonvarid, latvarid, levvarid, pfullvarid, ilevvarid, &
                       ...
                       tvarid, sphumvarid, liqwatvarid, o3varid, icvarid, & !emily
                       qrvarid, qsvarid, qgvarid   !emily
    integer(i_kind) :: icw,iql,iqi,iqr,iqs,iqg     !emily
...
!<<emily
    ...
    if (iqr>0) call gsi_bundlegetpointer(svalinc(ibin2),'qr', sub_qr, iret); istatus=istatus+iret  !emily
    if (iqs>0) call gsi_bundlegetpointer(svalinc(ibin2),'qs', sub_qs, iret); istatus=istatus+iret  !emily
    if (iqg>0) call gsi_bundlegetpointer(svalinc(ibin2),'qg', sub_qg, iret); istatus=istatus+iret  !emily
...
!<<emily
    ...
    if (iqr>0) call strip(sub_qr  ,qrsm  ,grd%nsig)  !emily
    if (iqs>0) call strip(sub_qs  ,qssm  ,grd%nsig)  !emily
    if (iqg>0) call strip(sub_qg  ,qgsm  ,grd%nsig)  !emily
